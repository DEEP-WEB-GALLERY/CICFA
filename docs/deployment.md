...
| Reading §10's in-flight WARN as "wait longer" | A Pages build can **hang**, not just run slow, and it looks identical from the outside. 2026-07-26, `fe8b288`: the build sat at `status: building` with `duration: 0` and `updated_at` never moving for **10 minutes**, while the last six builds took 30–47s. §10 WARNed "build probably still in flight" the whole time and would have gone red only after `DEPLOY_GRACE` — so the monitor's own advice ("re-run before believing it") quietly becomes an instruction to keep not knowing. The live site served the previous commit throughout. | Distinguish in-flight from stuck by asking the venue, not the clock: `gh api repos/DEEP-WEB-GALLERY/CICFA/pages/builds --jq '.[0]'`. A build whose `duration` is 0 and whose `updated_at` equals `created_at` well past ~60s is stuck. `gh api -X POST repos/DEEP-WEB-GALLERY/CICFA/pages/builds` requests a fresh one — it queued and completed in 39s, and parity went clean immediately. Safe to re-request; it publishes the same `main`. |
